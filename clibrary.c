...
void PrintUnsignedLongLong(unsigned long long Num, unsigned int Base, int FieldWidth, int ZeroPad, int LeftJustify, struct OutputStream *Stream)
{
    ...
    while (Num > 0)
    {
        unsigned long long NextNum = Num / Base;
        unsigned long long Digit = Num - NextNum * Base;
        if (Digit < 10)
            Result[--ResPos] = '0' + (char)Digit;
        else
            Result[--ResPos] = 'a' + (char)Digit - 10;
    ...
    for (FPos = Format; *FPos != '\0'; FPos++)
    {
        if (*FPos == '%')
        {
            ...
            switch (*FPos)
            {
                ...
                //add a pointer type for printf("%p"); - many pointer can be printed (as long long on WIN64)
                case 'p': FormatType = CharPtrType; break;
#ifndef NO_FP
                ...
                else
                {
                    ...
                    else
                    {
                        switch (*FPos)
                        {
                            ...
                            case 'p': PrintUnsignedLongLong(ExpressionCoerceUnsignedInteger(NextArg), 16, FieldWidth, ZeroPad, LeftJustify, Stream); break;
...
void LibFree(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
    ...
    if (Param[0]->Val->NativePointer != NULL)
    {
        /* ATT: if we release again - it crashes! */
        free(Param[0]->Val->NativePointer);
        ////Param[0]->Val->NativePointer = NULL;        /* it does not fix the issue */
    }
}
